...
const PRESET_COLORS = [
  '#f87171', '#fb923c', '#fbbf24', '#3ecf8e', '#34d399', '#2dd4bf',
...
export default function CategoriesPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (editing) { await update(editing.id, payload); toast('Categoria atualizada!') }
      else { await add(payload); toast('Categoria criada!') }
      setFormOpen(false)
    } catch (err: unknown) {
  ...
  async function handleDelete(cat: Category) {
    ...
    setDeleting(cat.id)
    try { await remove(cat.id); toast('Categoria excluída.') }
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs mt-0.5" style={{ color: 'var(--text-3)' }}>
            {unifiedMode ? 'Categorias da conta compartilhada' : 'Gerencie suas categorias'}
          </p>
        </div>
      ...
      {loading ? (
        ...
        <>
          <p className="text-xs -mt-3" style={{ color: 'var(--text-3)' }}>
            {displayCount} {displayCount === 1 ? 'categoria' : 'categorias'}
          </p>
      ...
      <Modal open={formOpen} onClose={() => setFormOpen(false)} title={editing ? 'Editar Categoria' : 'Nova Categoria'}>
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <Input label="Nome" placeholder="Ex: Alimentação" value={form.name}
            onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} required maxLength={60} />
          <div className="flex flex-col gap-1.5">
            <label className="text-[10px] font-semibold uppercase tracking-[2px]" style={{ color: 'var(--text-3)' }}>Tipo</label>
            <div className="flex gap-2">
              {(['expense', 'income', 'both'] as const).map((t) => (
          ...
          <div className="flex gap-3 pt-1">
            ...
            <Button type="submit" loading={saving} className="flex-[2]">{editing ? 'Salvar' : 'Criar categoria'}</Button>
          </div>
        </form>
      </Modal>
    </div>
...
function CategoryCard({ cat, isDeleting, onEdit, onDelete }: {
  cat: Category; isDeleting: boolean; onEdit: () => void; onDelete: () => void
}) {
  return (
    <div className="relative rounded-[16px] p-4 flex flex-col gap-3"
      style={{ background: 'rgba(18,18,28,0.55)', border: '1px solid rgba(255,255,255,0.09)',
      ...
      <div className="flex items-center gap-3 flex-1 min-w-0">
        ...
        <div className="min-w-0">
          ...
          <span className="text-[10px] font-medium" style={{ color: TYPE_COLOR[cat.type] }}>{TYPE_LABEL[cat.type]}</span>
        </div>
      </div>
