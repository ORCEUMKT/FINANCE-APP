...
export function Modal({ open, onClose, title, children, className }: ModalProps) {
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-end sm:items-center justify-center px-3 pt-0 pb-[80px] sm:p-4"
      style={{ background: 'rgba(5,5,10,0.8)', backdropFilter: 'blur(10px)' }}
      onClick={e => { if (e.target === e.currentTarget) onClose() }}
    >
      <div
        className={cn('w-full sm:max-w-lg rounded-[20px] max-h-[88vh] overflow-y-auto p-5 sm:p-6', className)}
        style={{
