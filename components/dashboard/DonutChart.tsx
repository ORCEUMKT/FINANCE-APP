...
export function DonutChart({ data, total, onCategoryClick }: DonutChartProps) {
  ...
  if (!total || data.length === 0) {
    return (
      <div className="flex items-center justify-center h-[180px] text-white/25 text-sm">
  ...
  const segments = data.map((d) => {
    ...
    return { d: `M${x1.toFixed(1)} ${y1.toFixed(1)} A${R} ${R} 0 ${lg} 1 ${x2.toFixed(1)} ${y2.toFixed(1)} L${x3.toFixed(1)} ${y3.toFixed(1)} A${r} ${r} 0 ${lg} 0 ${x4.toFixed(1)} ${y4.toFixed(1)}Z`, item: d }
  })
        ...
        <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="overflow-visible">
          {segments.map(({ d, item }) => (
            <path
              ...
              opacity={hovered && hovered !== (item.category_id ?? item.category_name) ? 0.35 : 1}
          ...
          <text x={cx} y={cy - 8} textAnchor="middle" fill="rgba(255,255,255,.3)" fontSize="9" fontWeight="700" letterSpacing="1">
            {hoveredItem ? hoveredItem.category_name.split('/')[0].trim().toUpperCase() : 'TOTAL'}
          </text>
          <text x={cx} y={cy + 11} textAnchor="middle" fill="#E8E8F0" fontSize="14" fontWeight="800">
            {formatCurrency(hoveredItem ? hoveredItem.total : total).replace(',00','')}
          </text>
          <text x={cx} y={cy + 26} textAnchor="middle" fill="rgba(255,255,255,.3)" fontSize="9">
      ...
      <div className="flex flex-wrap gap-x-4 gap-y-2 justify-center">
        {data.slice(0, 8).map((d) => (
          <button
            ...
            onMouseLeave={() => setHovered(null)}
            className="flex items-center gap-1.5 text-[10px] text-white/55 hover:text-white transition-colors"
          >
            <span className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ background: d.category_color }} />
            {d.category_name.split('/')[0].trim()}
            <span className="text-white/30">{d.percentage.toFixed(1)}%</span>
          </button>
        ))}
      </div>
    </div>
